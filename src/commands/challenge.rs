...
async fn challenge(ctx: &Context, msg: &Message, _: Args) -> CommandResult {
    let mut score = 0u32;
    let _ = msg.reply(ctx, "how much is 2+2").await;
    ...
    if let Some(answer) = &msg
        ...
        .await
    {
        if answer.content.parse::<i32>().unwrap() == 2 {
            let _ = answer.reply(ctx, "That's correct!").await;
            score += 1;
        } else {
            let _ = answer.reply(ctx, "Wrong, it's 4!").await;
        }
    } else {
    ...
    let react_msg = msg
        .reply(
            ctx,
            "React with the reaction representing 1, you got 10 seconds!",
        )
        .await
        .unwrap();
    ...
    let mut collector = collected
        .iter()
        .fold(builder, |b, msg| b.add_message_id(msg.id))
        .build()?;

    let _ = msg
        .reply(ctx, "Edit each of those 5 messages in 20 seconds")
        .await;
    ...
    if edited.len() >= 5 {
        ...
        let _ = msg
            .reply(ctx, &format!("You only edited {} out of 5", edited.len()))
            .await;
    }
    ...
    let _ = msg
        .reply(
            ctx,
            &format!(
                "TIME'S UP! You completed {} out of 4 tasks correctly!",
                score
            ),
        )
        .await;
